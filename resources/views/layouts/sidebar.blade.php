...
<aside class="main-sidebar sidebar-light-primary elevation-4">
    ...
    <a href="{{url('/')}}" class="brand-link">
      <img src="{{asset('images/logo.png')}}" alt="AdminLTE Logo" style="height:70px;margin: auto;" class="d-flex justify-content-center" >
      <!--<span class="brand-text" style="font-weight: 600">MarkOL</span>-->
    </a>
    ...
    <div class="sidebar">
      ...
      <div class="user-panel mt-3 pb-3 mb-3 d-flex">
        <div class="image">
          <img src="{{asset('storage/'.$profil->photo)}}" class="img-circle elevation-2" alt="User Image">
        </div>
        <div class="info">
          <a href="{{route('user.show',Auth::user()->id)}}" class="d-block">@if (Auth::check())
            {{Auth::user()->name}}
            @else
            Guest
          @endif</a>
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          <!-- Add icons to the links using the .nav-icon class
            ...
            @if (Auth::check())
            <li class="nav-item">
                <a href="{{route('logout')}}" class="nav-link" onclick="confirm('apakah anda ingin logout?'); event.preventDefault();
